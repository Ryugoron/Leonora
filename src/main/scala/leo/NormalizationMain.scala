...
object NormalizationMain {
  ...
  def main(args : Array[String]): Unit ={
    ...
    try {
      ...
      //Typdefinitionen
      var counter : Int = 0
      val rewriteF : Seq[FormulaStore] = rewrite.toSeq.map{l => {counter += 1; Store(counter.toString, Simp(Clause(l)), Role_Definition, 0, NoAnnotation)}}  // TODO is definition ok?
      val clauseF : Seq[FormulaStore] = clauses.toSeq.map{c => {counter += 1; Store(counter.toString, Simp(c), Role_Axiom,  0, NoAnnotation)}}
      val conjectureF : Seq[FormulaStore] = conjecture.toSeq.map{c => Store((counter+1).toString, Simp(c), Role_Conjecture,  0, NoAnnotation)}
...
//    }
  ...
  private def helpText : String = {
    ...
    sb.append("Leonora -- A Higher-Order Normalization Tool\n")
